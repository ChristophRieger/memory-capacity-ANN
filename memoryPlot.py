...
plt.title("Memorized patterns per N", fontsize=14)
...
plt.title("Memorized patterns per N squared", fontsize=14)
plt.legend(loc="upper right", fontsize=12)
...
plt.figure(figsize=(10,15))
plt.plot(N_simple, [i / j for i, j in zip(number_of_patterns_simple, [x**2 for x in N_simple] + N_simple)], marker = 'o', ms = 4, label='simple', color='blue')
plt.plot(N_custom_loss, [i / j for i, j in zip(number_of_patterns_custom_loss, [x**2 for x in N_custom_loss] + N_custom_loss)], marker = 'o', ms = 4, label='simple + custom loss', color='green')
plt.plot(N_recurrent, [i / j for i, j in zip(number_of_patterns_recurrent, [2 * x**2 for x in N_recurrent] + N_recurrent)], marker = 'o', ms = 4, label='recurrent layer (2 steps) + custom loss', color='red')
plt.plot(N_recurrent_2, [i / j for i, j in zip(number_of_patterns_recurrent_2, [2 * x**2 for x in N_recurrent_2] + N_recurrent_2)], marker = 'o', ms = 4, label='recurrent layer (3 steps) + custom loss', color='black')
plt.title("Memorized patterns per trainable parameter", fontsize=14)
plt.legend(loc="upper right", fontsize=12)
plt.ylabel("# of memorized patterns / trainable parameter", fontsize=13)
plt.xlabel("N", fontsize=13)
plt.tick_params(axis='both', labelsize=12)
plt.savefig("combindedPatternPerParameter" + ".png")